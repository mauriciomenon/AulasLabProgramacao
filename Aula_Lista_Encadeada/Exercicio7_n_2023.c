...
struct Item {
    int dado;  // informação do elemento
...
struct Lista {
    ...
    int maiorElemento;    // maior elemento na lista
};
...
void insercaoFinal(TLista* p, TItem x) {
    ...
    // Passo 7: atualizar a soma de todos os elementos
    (*p).soma += x.dado;

    // Passo 8: atualizar o maior elemento
    if (x.dado > (*p).maiorElemento) {
        (*p).maiorElemento = x.dado;
    }
}
...
int main() {
    ...
    printf("Digite uma sequencia de numeros inteiros (pressione Enter para encerrar a leitura):\n");

    // leitura da sequência de números inteiros
    char linha[100];
    ...
    // impressão da lista de inteiros e informações adicionais
    printf("\nNumeros digitados: ");
